...
@testset "HyperRectangle tests" begin
    ...
    @test WPB.HyperRectangle(pts) == WPB.HyperRectangle(SVector(-1.0, -1), SVector(1, 1.0))
    @test WPB.HyperRectangle(pts, true) ==
          WPB.HyperRectangle(SVector(-1.0, -1), SVector(1, 1.0))
    pts = SVector{2,Float64}[]
    for x in -1:0.1:1
        for y in -1:0.1:2
            push!(pts, SVector(x, y))
    ...
    @test WPB.HyperRectangle(pts) == WPB.HyperRectangle(SVector(-1.0, -1), SVector(1, 2.0))
    @test WPB.HyperRectangle(pts, true) ==
          WPB.HyperRectangle(SVector(-1.5, -1), SVector(3 / 2, 2.0))
