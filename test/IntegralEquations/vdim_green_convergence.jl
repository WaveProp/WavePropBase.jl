...
pde   = WPB.Laplace(;dim=2)
order = 1   # mesh order

Ψ_func = x -> cos(x[1])*sin(x[2])
∇Ψ_func = x -> SVector(-sin(x[1])*sin(x[2]),cos(x[1])*cos(x[2]))
...
for h in hh
    ...
    S₀ = WPB.SingleLayerOperator(pde,Ωₕ_quad,Γₕ_quad) |> Matrix
    D₀ = WPB.DoubleLayerOperator(pde,Ωₕ_quad,Γₕ_quad) |> Matrix
    ...
    V₀ = WPB.SingleLayerOperator(pde,Ωₕ_quad) |> Matrix
    approx0 = (V₀*f)
    er0 = ref - approx0
    @show norm(er0,Inf)
    ...
    δV = WPB.vdim_correction(pde,Ωₕ_quad,Ωₕ_quad,Γₕ_quad,S,D,V₀;location=:inside,order=porder)
    V = V₀ + δV
    ...
    @info length(Γₕ_quad.qnodes), length(Ωₕ_quad.qnodes), h
end

using Plots
plot(hh,ee0,xscale=:log10,yscale=:log10,label="no correction",m=:x,legend=:bottomright)
