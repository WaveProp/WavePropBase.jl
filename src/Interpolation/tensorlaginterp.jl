...
function TensorLagInterp(vals::Array{T,N}, nodes::NTuple{N,Vector{Td}}) where {N,Td,T}
    ...
    return TensorLagInterp{N,Td,T}(vals, nodes, weights)
end
...
@inline function bclag_interp(x::SVector{N,Td},
                              vals::Array{<:Any,N},
                              nodes::NTuple{N},
                              weights::NTuple{N},
                              ::Val{dim},
...
function cheb1nodes(n::NTuple{N}, rec::HyperRectangle{N}) where {N}
    return cheb1nodes(n, low_corner(rec), high_corner(rec))
end
...
function cheb1nodes_iter(n::NTuple{N}, rec::HyperRectangle{N}) where {N}
    return cheb1nodes_iter(n, low_corner(rec), high_corner(rec))
end
...
function cheb2nodes(n::NTuple{N}, rec::HyperRectangle{N}) where {N}
    return cheb2nodes(n, low_corner(rec), high_corner(rec))
end
...
function cheb2nodes_iter(n::NTuple{N}, rec::HyperRectangle{N}) where {N}
