...
"""
struct ReferencePoint <: AbstractReferenceShape{0} end
...
"""
struct ReferenceTriangle <: AbstractReferenceShape{2} end
...
"""
struct ReferenceHyperCube{N} <: AbstractReferenceShape{N} end
...
"""
struct ReferenceTetrahedron <: AbstractReferenceShape{3} end
function Base.in(x, ::ReferenceTetrahedron)
    return 0 ≤ x[1] ≤ 1 && 0 ≤ x[2] ≤ 1 - x[1] && 0 ≤ x[3] ≤ 1 - x[1] - x[2]
end
