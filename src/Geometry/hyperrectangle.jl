...
Works for any `el ∈ els` that implements `center(el)`.

```jldoctest
using StaticArrays
pts = [SVector(1,2), SVector(3.5,4), SVector(5,6)]
WavePropBase.bounding_box(pts)

# output

WavePropBase.HyperRectangle{2, Float64}([1.0, 2.0], [5.0, 6.0])
```
"""
