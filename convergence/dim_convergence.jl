...
for h in hh
    ...
    @show mesh
    xs = t == :interior ? center + WPB.svector(i->2*radius,N) : center + WBP.svector(i->0.5*radius,N)
    ...
    δS, δD = WPB.dim_correction(pde, mesh, mesh, S0, D0)
...
##
order = n + 1
fig = plot(hh,ee0,m=:x,label="nocorrection",yscale=:log10,xscale=:log10)
plot!(fig,hh,ee1,m=:x,label="dim correction")
